...
import { doc, getDoc, doc, setDoc, onSnapshot, collection } from 'firebase/firestore';

const SHARD_COLLECTION_ID = '_firebase_ext_/sharded_counter'; // '_counter_shards_';
...
export class Counter {
   ...
   */
  constructor(db: firebase.firestore.Firestore, firestoreDb: any, docPath: string, private field: string) {
    this.db = db;
    this.shardId = getShardId(COOKIE_NAME);

    firebase.initializeApp(this.db?.app?.options || {});

    const coll = collection(firestoreDb, SHARD_COLLECTION_ID);
    const shardsRef = coll; // firebase.firestore().collection(SHARD_COLLECTION_ID);
   ...
   */
  public incrementBy(val: number): Promise<void> | undefined {
    // @ts-ignore
    ...
    if (this.db) {
      const coll = collection(this.db, SHARD_COLLECTION_ID);
...
function getShardId(cookie: string): string {
  const result = new RegExp('(?:^|; )' + encodeURIComponent(cookie) + '=([^;]*)').exec(cookie);
  ...
  cookie = encodeURIComponent(cookie) + '=' + shardId + expires + '; path=/';
  return shardId;
}
