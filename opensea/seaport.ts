...
export class OpenSeaPort {
  ...
  public async transfer({
    ...
    const isOldNFT =
      isCryptoKitties ||
      (!!asset.version && [TokenStandardVersion.ERC721v1, TokenStandardVersion.ERC721v2].includes(asset.version));
  ...
  public async getFungibleTokens({
    ...
    address,
    name
  }: { symbol?: string; address?: string; name?: string } = {}): Promise<OpenSeaFungibleToken[]> {
  ...
  public async getTokenBalance(
    ...
    }: { accountAddress: string; tokenAddress: string; schemaName?: WyvernSchemaName },
    retries = 1
  ) {
    const asset: Asset = {
      tokenId: null,
  ...
  public async computeFees({
    ...
    if (sellerBountyBasisPoints > 0 && bountyTooLarge) {
      ...
      if (maxTotalBountyBPS >= OPENSEA_SELLER_BOUNTY_BASIS_POINTS) {
        errorMessage += ` Remember that we will add ${
          OPENSEA_SELLER_BOUNTY_BASIS_POINTS / 100
        }% for referrers with our accounts!`;
      }
  ...
  public async _getProxy(accountAddress: string, retries = 0): Promise<string | null> {
    let proxyAddress: string | null = await this._wyvernProtocolReadOnly.wyvernProxyRegistry.proxies.callAsync(
      accountAddress
    );
  ...
  public async _getStaticCallTargetAndExtraData({
    ...
    if (isCheezeWizards) {
      ...
      const cheezeWizardsBasicTournmentInstance = await cheezeWizardsBasicTournamentABI.at(
      ...
      return {
        ...
        staticExtradata: encodeCall(
          getMethod(StaticCheckCheezeWizards, 'succeedIfCurrentWizardFingerprintMatchesProvidedWizardFingerprint'),
  ...
  public async _makeBundleBuyOrder({
    ...
    const quantityBNs = quantities.map((quantity, i) =>
      WyvernProtocol.toBaseUnitAmount(makeBigNumber(quantity), assets[i].decimals || 0)
    );
  ...
  public async _makeBundleSellOrder({
    ...
    const { makerRelayerFee, takerRelayerFee, makerProtocolFee, takerProtocolFee, makerReferrerFee, feeRecipient } =
      this._getSellFeeParameters(
        totalBuyerFeeBasisPoints,
        totalSellerFeeBasisPoints,
        waitForHighestBid,
  ...
  public async _validateMatch(
    ...
    try {
      ...
      const canMatch = await requireOrdersCanMatch(this._getClientsForRead(retries).wyvernProtocol, {
        buy,
        sell,
        accountAddress
      });
      ...
      const calldataCanMatch = await requireOrderCalldataCanMatch(this._getClientsForRead(retries).wyvernProtocol, {
        buy,
        sell
      });
